locals {
  ...
  rbac_flat = flatten([
    for scope, rbacs in local.env_global_config.rbac : [
      for rbac in rbacs : {
      ...
      } if(scope != "resource_group") && (local.env_specific_config.primary_region ? true : scope != "subscription")
    ]
  ])
  ...
  rbac_combo_rg = [ # use setproduct to create all possible combinations of resource rbac roles and resourcegroups deployed
    for combo in setproduct(local.rbac_flat_rg, range(local.env_global_config.resource_groups)) : {
      rbac = combo[0]
      rg   = combo[1]
    }
  ]
...
resource "azurerm_resource_provider_registration" "this" {
  for_each = local.env_specific_config.primary_region ? local.providers : []
  name     = each.key
}

module "naming" {
  source       = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-naming?ref=1.0.9"
  product_area = local.env_global_config.auth_type
  ...
  generator = {
    domain = {
      resource_group         = local.env_global_config.resource_groups
      ...
      storage_account        = 2
      network_interface      = 3
      network_security_group = length(flatten(local.env_specific_config.vnets[*].subnets)) * 4
      private_endpoint       = 3
      route_table            = 1
    }
  }
...
module "resource_group_network" {
  ...
  resource_group_name = "${local.env_global_config.auth_type}-${local.env_specific_config.environment_location_id}-${local.env_global_config.app_short_name}-network-rg001"
...
module "resource_group_management" {
  source              = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-resource-group?ref=1.0.1"
  resource_group_name = "${local.env_global_config.auth_type}-${local.env_specific_config.environment_location_id}-${local.env_global_config.app_short_name}-management-rg001"
  location            = module.naming.location
  tags                = local.env_global_config.tags
}
...
module "resource_group_monitoring" {
  source              = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-resource-group?ref=1.0.1"
  resource_group_name = "${local.env_global_config.auth_type}-${local.env_specific_config.environment_location_id}-${local.env_global_config.app_short_name}-monitoring-rg001"
  location            = module.naming.location
  tags                = local.env_global_config.tags
}

module "resource_group" {
  count               = local.env_global_config.resource_groups
  source              = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-resource-group?ref=1.0.1"
