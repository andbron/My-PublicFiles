...
variable "product_area" {
  ...
  validation {
    condition = (
      ...
      && contains(["alz", "tcg", "aen"], var.product_area)
    )
    error_message = "Invalid 'product_area'. It must be exactly 3 characters long & match a prefefined set."
  }
}
...
variable "custom_abbreviation" {
  type        = string
  description = "To be used as custom abbreviation for custom services."
  validation {
    condition     = length(var.custom_abbreviation) >= 3 && length(var.custom_abbreviation) <= 5
    error_message = "Invalid 'custom_abbreviation'. It must be greater than 3 and less than 5 characters."
  }
  default = "abcde"
}