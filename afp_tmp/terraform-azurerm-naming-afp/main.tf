locals {
  resource_types = {
    ...
    data_factory_ssis = {
      name     = "data_factory_ssis"
      alphanum = true
      global   = true
      abbr     = "dfssis"
    }
    data_factory_shir = {
      name     = "data_factory_shir"
      alphanum = true
      global   = true
      abbr     = "dfshir"
    }
    ...
    virtual_machine_windows = {
      name     = "virtual_machine_windows"
      alphanum = false
      global   = false
      abbr     = "wsc"
    }
    virtual_machine_linux = {
      name     = "virtual_machine_linux"
      alphanum = false
      global   = false
      abbr     = "lxc"
    }
  ...
  pseudo_resource_types = {
    virtual_machine = {
      computer_name = {
        name            = "computer_name" # modern auth computer names
        alphanum        = true
        global          = true
    ...
    virtual_machine_windows = {
      computer_name_windows = {
        name            = "computer_name_windows" # modern auth computer names
        alphanum        = true
        global          = true
        abbr            = "" # Empty abbreviation
        max_name_length = 13
    ...
    virtual_machine_linux = {
      computer_name_linux = {
        name            = "computer_name_linux" # modern auth computer names
        alphanum        = true
        global          = true
        abbr            = "" # Empty abbreviation
        max_name_length = 13
  ...
  all_resource_types = merge(
    ...
    local.pseudo_resource_types.virtual_machine,
    local.pseudo_resource_types.virtual_machine_windows,
    local.pseudo_resource_types.virtual_machine_linux
  )

  location_abbr = lookup(var.location_map, var.location, "ae")
  env_abbr      = lookup(var.environment_map, var.environment, "np")
  prefix_parts  = [var.product_area, "${local.env_abbr}${local.location_abbr}"] # aen/tcg + np + ase
  affix_parts   = ["${local.env_abbr}${local.location_abbr}"]                   # np + ase
  prefix        = join("-", local.prefix_parts)
  affix         = join("-", local.affix_parts)
  domain        = var.domain
    ...
    domain, resources in var.generator : domain => { for
      type, count in merge(resources, local.pseudo_resources_generator[domain]) : type => {
        ...
        name_parts = compact(flatten([
          type == "computer_name_windows" ?
          ["wsc", "${local.env_abbr}${local.location_abbr}"] :
          type == "computer_name_linux" ?
          ["lxc", "${local.env_abbr}${local.location_abbr}"] :
          local.prefix_parts,
          local.domain,
