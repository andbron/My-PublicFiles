module "network_watcher" {
  source               = "git::https://dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-network-watcher?ref=1.0.1"
...
module "virtual_network" {
  ...
  source               = "git::https://dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-virtual-network?ref=1.0.1"
  resource_group_name  = module.resource_group_network.name
...
resource "azurerm_network_security_group" "private_endpoint" {
  for_each            = { for vnet in local.env_specific_config.vnets : vnet.symbolic_name => vnet }
  name                = "${module.naming.generated_names.domain.network_security_group[length(each.value.subnets) + 1]}-pe-${each.key}"
  location            = module.naming.location
  resource_group_name = module.resource_group_network.name
  tags                = local.env_global_config.tags
...
resource "azurerm_route_table" "this" {
  ...
  name                          = "tcg-${local.env_specific_config.environment_location_id}-${local.env_global_config.app_short_name}-${replace(each.key, "subnet", "rt00")}"
  location                      = module.naming.location
  resource_group_name           = module.resource_group_network.name
...
resource "azurerm_route_table" "private_endpoint" {
  ...
  name                          = "tcg-${local.env_specific_config.environment_location_id}-${local.env_global_config.app_short_name}-rt000-pe-${each.key}"
  location                      = module.naming.location
  resource_group_name           = module.resource_group_network.name
  disable_bgp_route_propagation = coalesce(var.disable_bgp_route_propagation, null)
  tags                          = local.env_global_config.tags
...
data "azurerm_virtual_network" "connectivity_hub" {
  ...
  name                = "tcg-${local.env_specific_config.environment_location_id}-conn-vnet001"
  resource_group_name = "tcg-${local.env_specific_config.environment_location_id}-conn-network-rg001"
}
...
resource "azurerm_virtual_network_peering" "peer_from_conn" {
  ...
  resource_group_name       = "tcg-${local.env_specific_config.environment_location_id}-conn-network-rg001"
  virtual_network_name      = "tcg-${local.env_specific_config.environment_location_id}-conn-vnet001"
...
module "private_endpoint_key_vault" {
  source                         = "git::https://dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-private-endpoint?ref=1.0.3"
  location                       = module.naming.location
...
module "private_endpoint_key_vault_states" {
  source                         = "git::https://dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-private-endpoint?ref=1.0.3"
  location                       = module.naming.location
  resource_group_name            = module.resource_group_network.name
  ...
  private_dns_zone_group_name    = "null"
  private_dns_zone_ids           = [data.azurerm_private_dns_zone.dns_zone["privatelink.vaultcore.azure.net"].id]
  private_connection_resource_id = module.key_vault_states.id
  subresource_names              = ["vault"]
  tags                           = local.env_global_config.tags
}

module "private_endpoint_storage_account_states" {
  source                         = "git::https://dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-private-endpoint?ref=1.0.3"
  location                       = module.naming.location
  resource_group_name            = module.resource_group_network.name
  ...
  private_dns_zone_group_name    = "null"
  private_dns_zone_ids           = [data.azurerm_private_dns_zone.dns_zone["privatelink.blob.core.windows.net"].id]
  private_connection_resource_id = module.storage_account_states.id
  subresource_names              = ["blob"]
  tags                           = local.env_global_config.tags
}
