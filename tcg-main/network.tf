...
module "virtual_network" {
  for_each             = { for vnet in local.vnets : vnet.symbolic_name => vnet }
  source               = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-virtual-network?ref=1.0.1"
  resource_group_name  = module.resource_group_network.name
  virtual_network_name = module.naming.generated_names.domain.virtual_network[index(local.vnets, each.value)]
...
resource "azurerm_network_security_group" "private_endpoint" {
  for_each            = { for vnet in local.vnets : vnet.symbolic_name => vnet }
  name                = "tcg-${local.environment_location_id}-${local.app_short_name}-nsg000-pe-${each.key}"
  location            = module.naming.location
  resource_group_name = module.resource_group_network.name
  tags                = local.env_global_config.tags
...
resource "azurerm_virtual_network_peering" "peer_to_conn" {
  for_each                  = { for vnet in local.vnets : vnet.symbolic_name => vnet }
  name                      = "${module.naming.generated_names.domain.virtual_network[index(local.vnets, each.value)]}-to-conn"
  resource_group_name       = module.resource_group_network.name
  virtual_network_name      = module.virtual_network[each.key].name
...
resource "azurerm_virtual_network_peering" "peer_from_conn" {
  ...
  name                      = "conn-to-${module.naming.generated_names.domain.virtual_network[index(local.vnets, each.value)]}"
  resource_group_name       = "tcg-${local.environment_location_id}-conn-network-rg001"
  virtual_network_name      = "tcg-${local.environment_location_id}-conn-vnet001"
...
data "azurerm_virtual_network" "connectivity_hub" {
  provider            = azurerm.connectivity
  name                = "tcg-${local.environment_location_id}-conn-vnet001"
  resource_group_name = "tcg-${local.environment_location_id}-conn-network-rg001"
}
...
module "private_endpoint_key_vault" {
  ...
  subnet_id                      = module.subnet_private_endpoint[local.vnets[0].symbolic_name].id
...
resource "azurerm_route_table" "this" {
  ...
  name                          = "tcg-${local.environment_location_id}-${local.app_short_name}-${replace(each.key, "subnet", "rt00")}"
  location                      = module.naming.location
  resource_group_name           = module.resource_group_network.name
...
resource "azurerm_route_table" "private_endpoint" {
  ...
  name                          = "tcg-${local.environment_location_id}-${local.app_short_name}-rt000-pe-${each.key}"
  location                      = module.naming.location
  resource_group_name           = module.resource_group_network.name
  disable_bgp_route_propagation = coalesce(var.disable_bgp_route_propagation, null)
  tags                          = local.env_global_config.tags
