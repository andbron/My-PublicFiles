data "azurerm_storage_account" "diagnostics" {
  ...
  name                = "tcg${local.environment_location_id}pbmnsa001"
  resource_group_name = "tcg-${local.environment_location_id}-pbmn-rg-storage-diag"
}
...
module "diagnostics_log_virtual_network" {
  ...
  source             = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-diagnostic-logs?ref=1.0.7"
  name               = "${module.naming.generated_names.domain.virtual_network[index(local.vnets, each.value)]}-diagnosticslogs-001"
...
module "diagnostics_log_network_security_group" {
  ...
  source             = "git::https://9025-CICD@dev.azure.com/9025-CICD/ESLZ%20Modules/_git/terraform-azurerm-diagnostic-logs?ref=1.0.7"
  name               = "${module.naming.generated_names.domain.network_security_group[index(local.network_subnets, each.value)]}-diagnosticslogs-001"
